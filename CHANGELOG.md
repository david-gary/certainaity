...
- `GANDetector` model (`src/forenscope/models/gandec.py`): CLIP ViT-L/14 backbone with global image-level classification head and local 16×16 patch map head.
...
- `publish.yml`: builds and pushes `forenscope-api` and `forenscope-worker` to GHCR on `v*` tags; creates GitHub Release from CHANGELOG.
