"""Celery task definitions for the Certainaity analysis pipeline.
...
from certainaity.report import ReportRegion, generate_report, save_json_report, save_pdf_report
from certainaity.worker.app import celery_app

if TYPE_CHECKING:
    from certainaity.models.ensemble import LocalizationResult
...
class _AnalysisTask(celery_app.Task):  # type: ignore[misc]
    ...
    def ensemble(self) -> object:
        if self._ensemble is None:
            from certainaity.models import Ensemble
...
@celery_app.task(
    bind=True,
    base=_AnalysisTask,
    name="certainaity.analyze_image",
    max_retries=2,
