...
The Dockerfile resolves ``certainaity.api.main:app`` as the ASGI entrypoint.
"""
...
from certainaity.api.metrics import REQUEST_LATENCY, REQUESTS_TOTAL
from certainaity.api.routes import router
...
async def _lifespan(application: FastAPI) -> AsyncGenerator[None, None]:
    ...
    log.info(
        "certainaity_api_starting",
        weights_dir=str(settings.weights_dir),
        output_dir=str(settings.output_dir),
        redis_url=settings.redis_url,
    )
    yield
    log.info("certainaity_api_stopping")


def create_app() -> FastAPI:
    """Construct and return the FastAPI application."""
    application = FastAPI(
        title="Certainaity API",
